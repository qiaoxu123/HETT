...
更新时间：2026-09-12 07:07 +08:00

## 当前结论

代码级和 CPU 数据审计已有明确进展，但所有方案的真实单卡导航效果尚未完成，因此总目标仍未完成。

按 2026-09-12 最新要求，当前只运行原版 seed 0、短跑筛选和后续完整 seed-0 矩阵。seed 17/42 多种子确认与依赖它的冻结后 test 已人工暂停；两个服务停止前都仅处于等待状态，额外种子运行数为 0。待完整 seed-0 结果评审并获得明确确认后再恢复，避免在方案尚未确定时浪费数周算力。
...
原版基线已完整跑完前 4 个 epoch，当前进入 epoch 5，最新记录为 2,100/10,939 batch（19.2%）；GPU、日志、磁盘均无告警。按目前每个完整 epoch 平均约 3.14 小时估算，原版剩余约 49.6 小时。它使用启动时源码快照，不受这些 worktree 提交影响。

梯度日志已明确标注为“ET 主体裁剪前范数”：epoch 1–4 的中位数依次为 18.79、30.40、43.58、47.89，最大值为 101.23、163.66、159.30、307.91；超过 100 的采样数为 1/111、5/111、6/111、10/111，但最长连续次数均不超过 2，所有记录的 loss 和范数仍为有限值。长尾在抬高，需要继续观察；ET 实际梯度会裁剪到 40，且该数值不覆盖语言/视觉两个独立优化器，所以目前不能判定发散。

在线监控进一步区分偶发尖峰和持续异常：batch loss 或梯度出现 NaN/Inf 会立即告警；裁剪前梯度连续三个采样点超过 100 才报告持续高梯度。单次超过 100 仍保留在报告中，但不会被误判为训练失败。

阶段诊断已加入逐 episode 和 epoch 汇总。epoch 1→4，val-seen SR/SPL 从 23.28%/17.35% 升至 30.89%/22.68%，NE 从 46.57m 降至 38.01m；val-unseen 则在 epoch 2–4 波动，当前最佳 SR 为 epoch 2 的 17.39%，最佳 NE 为 epoch 3 的 52.44m，尚无持续上升趋势。epoch 4 的 fine refinement 相对粗阶段终点将 seen SR 提高 8.70pp、NE 改善 2.05m，将 unseen SR 提高 4.56pp、NE 改善 2.66m，说明细化阶段此时确有正贡献。自动更新的图和逐轮报告保存在 `runs/chain_monitor_20260911/baseline_report/`。
