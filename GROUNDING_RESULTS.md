...
- 已固定 16 组 hard contrast：同一地图、完全相同且非空的 landmark 列表、不同目标对象，目标间至少相距 20 米；四张 unseen 地图轮流取样。
- hard contrast 将保持 RGB、pose 和 landmark prior 不变，只交换成配对目标的指令；比较 t=0 目标预测以及完整导航，避免“打乱后场景也变了”的混淆。
- 完整 ET 已用 batch=2、240×240 地图、49 个区域执行前向和反向；六项输出形状正确，region grounding 全部参数获得有限梯度。关闭模块时仍返回一个历史 token，兼容原路径。
...
- 同地标不同目标、整句打乱、视觉打乱、关闭模块的 checkpoint 闭环导航消融。
