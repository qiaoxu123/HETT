...
- [ ] 多种子人工确认门：根据用户要求，seed 17/42 和冻结后 test 已暂停且尚未产生运行数据；完整 seed-0 矩阵评审并得到明确确认后才恢复。
- [x] 实际参数审计：每个完整训练的 `training_args.json` 必须逐项匹配预声明 seed、20 epochs、单卡 batch/累积、优化器、论文参数、loss 消融权重及模块开关。
- [x] 三种子汇总器：不挑最佳 seed，自动保存均值、样本标准差、95% 区间、逐 seed 配对变化，并绘制总体指标和配对差值图。
- [x] 失败案例索引：三种子汇总自动列出最差终点、到达后丢失、错误停止、配对救回/退化样本并绘制失败类型比例；原始轨迹仍是权威数据。
...
- [x] 长队列磁盘预算：按真实 1.9GB checkpoint 估算容量；seed 17/42 的 20 个运行落到 `dataext`，原实验路径用透明软链接保留，审计按真实文件解析。
- [x] 在线数值健康检查：NaN/Inf 立即告警；裁剪前梯度连续三个采样点超过 100 才标记持续异常，单个尖峰仅记录。
...
- [x] 机器完工审计：最终服务只有在全部队列、epoch、checkpoint 内容重新计算后与记录哈希一致、预测、历史基线可比性、三 seed 固定提交与源码哈希一致、点云负结果、无 test 泄漏、卫生补丁哈希、冻结顺序及图表检查通过后才允许标记完成；最终项数随冻结方案动态确定。
