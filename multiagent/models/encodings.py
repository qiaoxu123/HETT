...
class PosEncoding(nn.Module):
    ...
    def forward(self, lang, frames, directions, len_lang, pos=None):
        if pos is None:
...
class MapPosEncoding(nn.Module):
    """
    Transformer-style positional encoding with wavelets
    """
    ...
    def forward(self, lang, frames, directions, maps, positions, len_lang, pos=None):
        if pos is None:
            enc = self.pe[:, : lang.shape[1] + frames.shape[1] + positions.shape[1]]
        else:
            enc = [[] for _ in range(len(lang))]
            for batch_idx in range(pos.shape[0]):
                for pos_idx in range(lang.shape[1] + positions.shape[1]):
                    enc[batch_idx].append(self.pe[0, pos[batch_idx, pos_idx]])
            enc = torch.stack([torch.stack(pos_batch) for pos_batch in enc])
        enc = enc / math.sqrt(self.d_model)
        lang = lang + enc[:, : lang.shape[1]]
...
class CanPosEncoding(nn.Module):
    """
    Transformer-style positional encoding with wavelets
    """
    ...
    def forward(self, lang, frames, directions, maps, positions, centroids, len_lang, pos=None):
        if pos is None:
            enc = self.pe[:, : lang.shape[1] + frames.shape[1] + positions.shape[1] + centroids.shape[1]]
        else:
            enc = [[] for _ in range(len(lang))]
            for batch_idx in range(pos.shape[0]):
                for pos_idx in range(lang.shape[1] + positions.shape[1]):
...
class PosLearnedEncoding(nn.Module):
    ...
    def forward(self, lang, frames, directions, lens_lang, lens_frames):
