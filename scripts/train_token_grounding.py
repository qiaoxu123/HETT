...
from multiagent.models.token_grounding_heatmap import TokenGroundingHeatmap  # noqa: E402

SIZE = 64
MAX_LANDMARKS = 4
MAX_TOKENS = 96
MAX_NAME_TOKENS = 16
BERT_PATH = "/home/tenant2/dataext/home-cache/huggingface/hub/models--bert-base-uncased/snapshots/86b5e0934494bd15c9632b12f734a8a67f723594"
SIGMA_METERS = 10.0
NMS_METERS = 20.0
...
class HeatmapDataset(Dataset):
    def __init__(self, samples, tokenizer, basis_cache):
        self.samples = samples
        self.basis_cache = basis_cache
        self.pair_cache = {}
        for sample in self.samples:
            instruction = tokenizer(sample["description"], max_length=MAX_TOKENS,
                                    padding="max_length", truncation=True)
            sample["instruction_ids"] = np.asarray(instruction["input_ids"], dtype=np.int64)
            sample["instruction_mask"] = np.asarray(instruction["attention_mask"], dtype=np.int64)
            names = sample["names"] or [""]
            name_tokens = tokenizer(names, max_length=MAX_NAME_TOKENS,
                                    padding="max_length", truncation=True)
            sample["name_ids"] = np.zeros((MAX_LANDMARKS, MAX_NAME_TOKENS), dtype=np.int64)
            sample["name_mask"] = np.zeros((MAX_LANDMARKS, MAX_NAME_TOKENS), dtype=np.int64)
            count = min(len(sample["names"]), MAX_LANDMARKS)
            if count:
                sample["name_ids"][:count] = np.asarray(name_tokens["input_ids"][:count])
                masks = np.asarray(name_tokens["attention_mask"][:count])
                # Exclude CLS/SEP so the query represents the landmark words.
                masks[:, 0] = 0
                lengths = masks.sum(axis=1)
                for row, length in enumerate(lengths):
                    if length:
                        masks[row, int(length)] = 0
                sample["name_mask"][:count] = masks
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "instruction_mask": sample["instruction_mask"],
            "name_ids": sample["name_ids"], "name_mask": sample["name_mask"], "valid": valid,
...
def forward(model, batch):
    return model(batch["angle_fields"].float(), batch["distance_fields"].float(),
                 batch["pair"].float(), batch["valid"], batch["instruction_ids"],
                 batch["instruction_mask"], batch["name_ids"], batch["name_mask"])
...
def evaluate(model, loader, dataset, device):
    ...
    with torch.no_grad():
        for raw_batch in loader:
            ...
            logits, relation_weights, pair_gate, attention = forward(model, batch)
            ...
            for offset, sample_index in enumerate(raw_batch["index"]):
                ...
                outputs.append({
                    ...
                    "attention_entropy": float((-(attention[offset].clamp_min(1e-8).log() * attention[offset]).sum(-1)
                                                 * batch["valid"][offset]).sum().cpu() /
                                                batch["valid"][offset].sum().clamp_min(1)),
...
def main():
    ...
    parser.add_argument("--bert-path", default=BERT_PATH)
    ...
    datasets = {name: HeatmapDataset(samples, tokenizer, basis_cache) for name, samples in splits.items()}
    loaders = {
    ...
    model = TokenGroundingHeatmap(args.bert_path, MAX_LANDMARKS,
    ...
    for epoch in range(args.epochs):
        ...
        for raw_batch in loaders["train_seen"]:
            batch = move(raw_batch, device)
            logits, _, _, _ = forward(model, batch)
    ...
    result = {
        ...
        "device": str(device), "bert_path": args.bert_path,
    ...
    torch.save({"model": {key: value for key, value in model.state_dict().items()
                           if not key.startswith("bert.")}, "bert_path": args.bert_path,
