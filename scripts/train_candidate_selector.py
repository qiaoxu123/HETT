...
from multiagent.models.relational_heatmap import GeometricCandidateSelector  # noqa: E402
from scripts.train_relational_heatmap import (  # noqa: E402
    ...
    Vocabulary, landmark_basis, load_objects, load_split, pair_field, replace_names,
)
...
class CandidateDataset(Dataset):
    def __init__(self, samples, vocab, candidate_cache):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "ref_tokens": ref_tokens, "global_tokens": sample["global_tokens"],
...
def evaluate(model, loader, dataset, device):
    ...
    with torch.no_grad():
        for raw in loader:
            ...
            logits = model(batch["ref_tokens"], batch["global_tokens"],
...
def main():
    ...
    parser.add_argument("--val-limit", type=int, default=0); args = parser.parse_args()
    ...
    for sample in splits["train_seen"]:
        texts.append(replace_names(sample["description"], sample["names"]))
        texts.extend(replace_names(sample["description"], sample["names"], selected=index)
                     for index in range(len(sample["names"])))
    vocab = Vocabulary(texts); cache = {}
    ...
    datasets = {key: CandidateDataset(value, vocab, cache) for key, value in splits.items()}
    loaders = {
    ...
    model = GeometricCandidateSelector(len(vocab.itos), MAX_LANDMARKS, CANDIDATES_PER_LANDMARK).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for raw in loaders["train_seen"]:
            batch = move(raw, device); logits = model(batch["ref_tokens"], batch["global_tokens"], batch["landmark_valid"], batch["pair_valid"])
    ...
    torch.save({"model": model.state_dict(), "vocab": vocab.itos}, args.run_dir / "model.pt")
