"""Paired same-landmark/different-target evaluation with instruction swaps."""
...
def landmark_key(objects, trajectory):
    description = objects[trajectory.map_name][trajectory.object_id].processed_descriptions[trajectory.desc_id]
    return tuple(sorted(name.strip() for name in description.landmarks))


def select_pairs(split, count, seed=20260911):
    """Select pairs using labels only for evaluation-set construction."""
    ...
    for trajectory in load_mturk_trajectories(split, 'all', 50.0):
        landmarks = landmark_key(objects, trajectory)
        if landmarks:
            groups[(trajectory.map_name, landmarks)].append(trajectory)
    ...
    for (map_name, landmarks), entries in sorted(groups.items()):
        ...
        for index, left in enumerate(entries):
            ...
            if right is not None:
                used.update((left_id, (right.map_name, right.object_id, right.desc_id)))
                by_map[map_name].append((left, right, landmarks))
    for candidates in by_map.values():
...
def pair_manifest(objects, pairs):
    ...
    for left, right, landmarks in pairs:
        def details(item):
            ...
            return {
                'id': [item.map_name, item.object_id, item.desc_id],
...
def evaluate(output, checkpoint, objects, pairs):
    ...
    try:
        ...
        for left, right, _ in pairs:
            left_obj = objects[left.map_name][left.object_id]
            right_obj = objects[right.map_name][right.object_id]
    ...
    result = {
        ...
        'mean_first_prediction_shift_m': float(np.mean([row['first_prediction_shift_m'] for row in rows])),
    ...
    ax.hist(advantages, bins=15, color='#2563eb')
    ax.axvline(0, color='black', linewidth=1)
    ax.set_xlabel('correct-instruction advantage at t=0 (m)')
    ax.set_title('Same landmark, different target')
    fig.savefig(output / 'contrast.png', dpi=180)
...
def main():
    ...
    manifest = {
        'split': 'val_unseen',
        'seed': args.seed,
        'selection': 'same map and landmark list; distinct targets at least 20m apart',
