...
def load_or_create_embeddings(texts, cache_path, device):
    if cache_path.exists():
        cache = torch.load(cache_path, map_location="cpu", weights_only=False)
        if (cache.get("encoder_source") != "bert-base-uncased-pristine" or cache["texts"] != texts
    ...
    print(f"encoding {len(texts)} unique marked instructions with pristine BERT", flush=True)
    ...
    torch.save({"texts": texts, "embeddings": embeddings,
                "encoder_source": "bert-base-uncased-pristine",
...
def main():
    ...
    embeddings = load_or_create_embeddings(texts, args.embedding_cache, device)
    ...
    torch.save({"model": model.state_dict(), "encoder_source": "bert-base-uncased-pristine"},
