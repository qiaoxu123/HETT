...
def build_commands(snapshot, run, args):
    if args.custom_eval_script:
        evaluation = [args.python, str(snapshot / args.custom_eval_script),
                      '--run-dir', str(run)] + list(args.custom_eval_arg)
        return [], evaluation
    ...
    if getattr(args, 'resume_from', None):
        train += ['--checkpoint', str(Path(args.resume_from).resolve()), '--resume_optimizer']
...
def main():
    ...
    parser.add_argument('--custom-eval-script',
                        help='Repository-relative Python script for a provenance-tracked eval')
    parser.add_argument('--custom-eval-arg', action='append', default=[])
    ...
    if args.phase == 'eval' and not args.checkpoint and not args.custom_eval_script:
    ...
    atomic_json(run / 'status.json', dict(time=stamp(), phase='waiting_gpu', lock_file=str(lock_path)))
