...
RUNS = [ROOT / "runs" / f"base_bert_selector_s{index}" for index in range(3)]
...
def main():
    ...
    output = ROOT / "runs" / "base_bert_selector_aggregate.json"
    output.write_text(json.dumps(aggregate, ensure_ascii=False, indent=2))

    lines = ["# 原始 BERT token-mean 几何候选选择器：三 seed 汇总", "",
    ...
    (ROOT / "runs" / "BASE_BERT_SELECTOR_AGGREGATE.md").write_text("\n".join(lines) + "\n")
    ...
    ax.legend(); fig.tight_layout(); fig.savefig(ROOT / "runs" / "base_bert_selector_relations.png", dpi=180)
