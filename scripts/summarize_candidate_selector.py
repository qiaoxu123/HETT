...
RUNS = [ROOT / "runs" / f"geometric_selector_s{index}" for index in range(3)]
...
def main():
    ...
    output = ROOT / "runs" / "geometric_selector_aggregate.json"
    output.write_text(json.dumps(aggregate, ensure_ascii=False, indent=2))

    lines = ["# 显式几何候选选择器：三 seed 汇总", "",
    ...
    (ROOT / "runs" / "GEOMETRIC_SELECTOR_AGGREGATE.md").write_text("\n".join(lines) + "\n")
    ...
    ax.legend(); fig.tight_layout(); fig.savefig(ROOT / "runs" / "geometric_selector_relations.png", dpi=180)
